...
public class Hodgkin_Huxley_Nernst implements Serializable {
	...
	public I_ChannelHHNernst create(
		@ParamInfo(name = "", style = "default", options = "ugx_globalTag=\"gridFile\"; fct_tag=\"fctDef\"; type=\"S3:potential, [K], [Na]\"") UserDataTuple hhData
	) {
		String[] selFct = ((UserDependentSubsetModel.FSDataType) hhData.getData(0)).getSelFct();
		if (selFct.length != 3) {
			throw new RuntimeException("Hodgkin-Huxley (Nernst type) channel mechanism needs exactly three functions, but has " + selFct.length + ".");
		}

		String[] selSs = ((UserDependentSubsetModel.FSDataType) hhData.getData(0)).getSelSs();
		if (selSs.length == 0) {
			throw new RuntimeException("No subset definition in Hodgkin-Huxley (Nernst type) channel definition!");
		}
	 ...
	 * @param condData
	 */
	@MethodInfo(name = "set conductances glob", interactive = false)
	public void set_concuctances_glob(
		@ParamInfo(name = " ", style = "array", options = "type=\"s|n|n:subset glob,gK [S/m^2],gNa [S/m^2]\"") UserDataTuple[] condData
	) {
		check_channel_exists();
		for (UserDataTuple cD : condData) {
			String subsetGlobString = cD.getSubset(0);
			for (String subset : selectedSubsets) {
				if (subset.matches(subsetGlobString)) {

					if (!(cD.getNumberData(1) instanceof I_ConstUserNumber)) {
					...
					if (!(cD.getNumberData(2) instanceof I_ConstUserNumber)) {
						eu.mihosoft.vrl.system.VMessage.exception("Invalid specification: ",
							"Hodgkin-Huxley channel conductance cannot be given as code.");
					}

					double gK = ((Const__I_ConstUserNumber) cD.getNumberData(1)).const__get();
					double gNa = ((Const__I_ConstUserNumber) cD.getNumberData(2)).const__get();

					check_value(gK);
					check_value(gNa);

					hhChannel.set_conductances(gK, gNa, subset);
				}
			}
		}
	}
