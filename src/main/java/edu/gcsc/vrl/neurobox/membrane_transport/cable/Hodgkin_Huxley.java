...
/**
 * Hodgkin-Huxley mechanism This class is a VRL visualization for the
 * Hodgkin-Huxley transport implementation usable in a 1D cable equation.
 *
 * @author mbreit
 * @date 22-06-2015
 */
...
public class Hodgkin_Huxley implements Serializable {
	...
	public I_ChannelHH create(
		@ParamInfo(name = "", style = "default", options = "ugx_globalTag=\"gridFile\"; fct_tag=\"fctDef\"; type=\"S1:membrane potential\"") UserDataTuple hhData
	) {
		String[] selFct = ((UserDependentSubsetModel.FSDataType) hhData.getData(0)).getSelFct();
		if (selFct.length != 1) {
			throw new RuntimeException("Hodgkin-Huxley channel mechanism needs exactly one function, but has " + selFct.length + ".");
		}

		String[] selSs = ((UserDependentSubsetModel.FSDataType) hhData.getData(0)).getSelSs();
		if (selSs.length == 0) {
			throw new RuntimeException("No subset definition in Hodgkin-Huxley channel definition!");
		}
	...
	/**
	 * @brief supply a glob string to set conductance for subsets
	 * This will select by a globbing some subsets matching a given pattern,
	 * then will set the same conductance for all subsets in this group
	 * if and only if the channel has actually been installed on the subset
	 * @author sgrein
	 * @param hhData
	 */
	@MethodInfo(name = "set concuctances glob", interactive = false)
	public void set_conductances_glob(
		@ParamInfo(name = "Subset glob: ", 
			   style = "array", 
			   options = "type=\"s|n|n:subet glob, gK [S/m^2], gNA [S/m^2]\"") 
			   UserDataTuple[] hhData
	) {
		check_channel_exists();
		
		for (UserDataTuple hD : hhData) {
			String subsetGlobString = hD.getSubset(0);
			for (String subset : selectedSubsets) {
				if (subset.matches(subsetGlobString)) {
					String selSs = hD.getSubset(0);
					if (!(hD.getNumberData(1) instanceof I_ConstUserNumber)) {
					...
					if (!(hD.getNumberData(2) instanceof I_ConstUserNumber)) {
						eu.mihosoft.vrl.system.VMessage.exception("Invalid specification: ",
							"Hodgkin-Huxley channel conductance cannot be given as code.");
					}

					double gK = ((Const__I_ConstUserNumber) hD.getNumberData(1)).const__get();
					double gNa = ((Const__I_ConstUserNumber) hD.getNumberData(2)).const__get();
